...
namespace AOCHelper
{
    ...
    public class Helper
    {
        ...
        public static void VisualizeMapFile(Dictionary<long, Dictionary<long, Item>> map)
        {
            ...
            File.WriteAllText("out222.txt", sb.ToString());
        }

        public static void VisualizeMapInFile(Dictionary<long, Dictionary<long, Item>> map, int maxx, int maxy)
        {
            var formatStr = string.Concat(Enumerable.Range(0, map[0].Count).Select(i => "|{"+i+",5}"));

            StringBuilder sb = new StringBuilder();
            foreach (var y in map.OrderBy(k=>k.Key))
            {
