...
namespace AOCHelper
{
    ...
    public class Item
    {
        ...
        public bool Trench { get; set; } = false;
    ...
    public class Helper
    {
        ...
        public static Dictionary<long, Dictionary<long, Item>> ParseInput(List<string> input)
        {
            ...
            for (int i = -1 * input.Count + 1; i < input.Count; i++)
            {
                ret.Add(i, new Dictionary<long, Item>());
                for (int j = -c + 1; j < c; j++)
                {
                    var item = new Item { Coordinate = new Coordinate { X = j, Y = i }, ValueCh = '.' };
                    ret[i].Add(j, item);
                }
            }
        ...
        public static void VisualizeMapInFile(Dictionary<long, Dictionary<long, Item>> map)
        {
